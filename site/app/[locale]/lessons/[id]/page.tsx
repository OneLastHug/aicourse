...
import { course, pick, neighbors } from "@/lib/content";
...
export function generateStaticParams() {
  return course.outline.lessons.map((l) => ({ id: l.id }));
}
...
export default async function LessonPage({
  ...
  // Highlight each step's real code snippet once (server-side).
  ...
  const { prev, next, index, total } = neighbors(id);
    ...
    <article className="animate-fadeUp px-5 py-8 sm:px-8 lg:px-12">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-2 flex items-center gap-2 text-xs text-ink-faint dark:text-zinc-500">
          <Link href={`/${loc}`} className="hover:text-brand">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-2 text-xs text-ink-faint dark:text-zinc-500">
        ...
        <nav className="mt-10 flex items-stretch gap-3 border-t border-line pt-6 dark:border-zinc-800">
          {prev ? (
            <Link
              href={`/${loc}/lessons/${prev.id}`}
              className="card flex-1 p-3 transition hover:-translate-y-0.5"
            >
          ...
          {next ? (
            <Link
              href={`/${loc}/lessons/${next.id}`}
              className="card flex-1 p-3 text-right transition hover:-translate-y-0.5"
            >
