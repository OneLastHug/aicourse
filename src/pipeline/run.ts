...
/** v2 pipeline (Chinese-first): ingest → analyze → curriculum → lessons(2-phase, Chinese)
 *  → validate×2 → translate ZH→EN. On any failure the cloned repo is removed. */
export async function runPipeline(args: {
  ...
  try {
    ...
    if (cfg.validate) {
      ...
      const r1 = await validateCorrectness({ zhCourse, driver, cfg, cache, onProgress });
      onProgress?.({ type: "stage", stage: "validate2", label: "Validation 2 · alignment with the repo" });
      const r2 = await validateAlignment({ ctx, zhCourse, driver, cfg, cache, onProgress });
    ...
    onProgress?.({ type: "stage", stage: "translate", label: "Translating the Chinese course to English" });
    const course = await runTranslateStage({ zhCourse, driver, cfg, cache, onProgress });
