...
export async function runTranslateStage(args: {
  ...
  const wholeKey = cache.key({ stage: "translate", sha, n: flat.length, cfg: fp, v: 8 });
  ...
  const outlineKey = cache.key({ stage: "translate.outline", sha, cfg: fp, v: 4 });
  ...
  const entries = await Promise.all(flat.map((l) => limit(async () => {
    ...
    const lessonKey = cache.key({ stage: "translate.lesson", sha, id: l.id, cfg: fp, v: 6 });
