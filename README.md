...
> 后端用 **codex**（gpt-5.4 / xhigh）拉取并拆解仓库，教程生成共享 **10** 槽并发池，结合真实代码逐课编写。

```text
首页输入框 ──► 后端拉取仓库 ──► codex 分层拆解(s01→sN) ──► 并发子agent填充(共享10池) ──► 实时进度 ──► 教程页
```
...
- 🧠 **codex 编排**：隔离环境中的 `codex exec`，模型 `gpt-5.4`，思考强度 `xhigh`，教程生成共享 **10** 槽并发池。
...
```text
codex exec --model gpt-5.4 -c model_reasoning_effort=xhigh -C <repo> --output-last-message <file> "<prompt>"
```
